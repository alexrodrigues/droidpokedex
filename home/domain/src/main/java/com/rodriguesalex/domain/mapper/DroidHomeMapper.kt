...
fun PokemonListDetailedItemResponse.toModel(): PokemonListItem =
    PokemonListItem(
        ...
        backgroundColor = DroidPokemonTypeColor.getPokemonColor(
...
fun AbilitySlotResponse.toModel() = AbilitySlot(
    isHidden = is_hidden,
    slot = slot,
    ability = ability.toModel()
)

fun NamedAPIResourceResponse.toModel() = NamedAPIResource(
    name = name,
    url = url
)

fun GameIndexResponse.toModel() = GameIndex(
    gameIndex = game_index,
    version = version.toModel()
)

fun HeldItemResponse.toModel() = HeldItem(
    item = item.toModel(),
    versionDetails = version_details.map { it.toModel() }
)

fun HeldItemVersionResponse.toModel() = HeldItemVersion(
    version = version.toModel(),
    rarity = rarity
)

fun MoveSlotResponse.toModel() = MoveSlot(
    move = move.toModel(),
    versionGroupDetails = version_group_details.map { it.toModel() }
)

fun MoveVersionResponse.toModel() = MoveVersion(
    moveLearnMethod = move_learn_method.toModel(),
    versionGroup = version_group.toModel(),
    levelLearnedAt = level_learned_at
)

fun SpritesResponse.toModel() = Sprites(
...
fun StatSlotResponse.toModel() = StatSlot(
    stat = stat.toModel(),
    effort = effort,
    baseStat = base_stat
)

fun TypeSlotResponse.toModel() = TypeSlot(
    slot = slot,
    type = type.toModel()
)
